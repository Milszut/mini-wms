...
export default function MobileFilterPanel({ isOpen, onClose, filters, setFilters, sort, setSort, locations = [], services = [], conditions = [], onReset,}) {
  const [open, setOpen] = useState(null);
  const checkboxSources = {
    service: { label: "Dział", data: services, selectAll: true },
    condition: { label: "Stan", data: conditions, selectAll: false },
    location: { label: "Lokalizacja", data: locations, selectAll: true },
  };
  ...
  const defaults = {
    service: services.map((x) => String(x.id)),
    condition: conditions.map((x) => String(x.id)),
    location: locations.map((x) => String(x.id)),
  };
              ...
              <div key={key}>
                ...
                <div className={`transition-all overflow-hidden ${open === key ? "max-h-[900px] mt-2" : "max-h-0"}`}>
                  <div className="flex flex-col px-1">
                    {cfg.selectAll && (
                        <div
                        className="bg-white/15 px-3 py-2 cursor-pointer"
                        onClick={() => {
                            const all = cfg.data.map((x) => String(x.id));
                            const next =
                            filters[key].length === cfg.data.length ? [] : all;
                            setFilters((f) => ({ ...f, [key]: next }));
                        }}
                        >
                            <div className="flex flex-row items-center gap-2">
                                {filters[key].length === cfg.data.length ? (
                            ...
                            <div className="w-full h-px bg-white/15 mt-2"></div>
                        </div>
                    )}
              ...
              <div key={key}>
                ...
                <div className={`transition-all px-1 overflow-hidden ${open === key ? "max-h-[600px] mt-2" : "max-h-0"}`}>
                  <div className="flex flex-col">
                    {cfg.data.map((opt) => {const checked = sort[key] === opt.value;
