...
export const PopUpEdit = ({title = "Edytuj", initialValue = "", submitText = "Zapisz", onSubmit, onClose,}) => {
  const [value, setValue] = useState(initialValue);
  const handleOnClose = (e) => {
    if (e.target.id === "popup") onClose();
  };

  const isDisabled = !value.trim() || value.trim() === initialValue.trim();
    ...
    >
      <div className="bg-[#203047] shadow-black rounded-xl p-6 w-auto min-w-sm flex flex-col items-center text-center shadow-xl">
        ...
        <input
          ...
          className="w-full p-2 mb-5 text-white placeholder-grey-500 rounded border border-2  bg-[#18202D] border-white focus:outline-none focus:border-blue-700 hover:border-blue-700"
        />
